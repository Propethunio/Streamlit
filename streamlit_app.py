...
# Trwałe zaciąganie historii RPG z bazy danych SQLite
...
def render_personality_selector():
    persona = st.selectbox("Osobowość asystenta:", [
...
with st.sidebar:
    ...
    st.markdown(f"""
        <div class="token-counter" style="margin-top: 10px;">
            📊 SESJA: {st.session_state.total_tokens} tokenów
        </div>
    """, unsafe_allow_html=True)
...
# RENDEROWANIE OKNA ROZGRYWKI RPG
    ...
    if not character:
        st.info("Brak aktywnego bohatera w bazie danych. Stwórz postać z poziomu tego okna, aby rozpocząć permanentną kampanię.")
        ...
        # Wyświetlanie aktualnego obrazu lokacji wygenerowanego przez DALL-E
        ...
        # Trwałe renderowanie zapisanej historii kampanii bezpośrednio z SQLite
        ...
        # Reakcja silnika LLM na ruch gracza
        if st.session_state.rpg_messages and st.session_state.rpg_messages[-1]["role"] == "user":
            rpg_system_prompt = (
                ...
                f"Gracz steruje postacią {character['name']} (klasa: {character['class']}). "
                "Generuj rozbudowane i głębokie opisy środowiska, reaguj logicznie na poczynania gracza. "
                "Na końcu swojej wypowiedzi wypisz dokładnie 3 ponumerowane lub oznaczone literami opcje zachowania (A, B, C)."
            )
            ...
            with st.chat_message("assistant", avatar="🧙‍♂️"):
                status = st.empty()
                status.markdown('<div class="thinking-box"><div class="loader"></div><span>Mistrz Gry pisze kolejny rozdział...</span></div>', unsafe_allow_html=True)
                ...
                try:
                    ...
                    # Zapis wypowiedzi MG do SQLite
                    rpg_database.save_chat_message("assistant", full_rpg_response)
                    
                    # Generowanie nowej grafiki świata
                    with st.spinner("🖼️ Wizualizacja nowej lokacji cyfrowej..."):
                        img_url = rpg_visuals.generate_game_scene(client, full_rpg_response)
                        if img_url: st.session_state.last_rpg_image = img_url
                    
                    # Aktualizacja dynamiczna statystyk bohatera w bazie
                    next_hp = max(15, character['hp'] - 4)
                    next_gold = character['gold'] + 5
