...
def build_rpg_system_prompt(character, inventory_list, lore_text):
    ...
    return (
        ...
        f"4.4. Cios potencjalnie śmiertelny (hp_change sprowadzający HP do 0 lub poniżej) stosuj przy jednoznacznie śmiertelnych akcjach.\n"
        f"4.5. Jeśli gracz CELOWO wybiera akcję jednoznacznie śmiertelną (skok z wieżowca, wypicie trucizny, dobrowolna kapitulacja na śmierć) — "
        f"NIE RATUJ GO. Nie wymyślaj przypadkowych ocalenia ('implant w ostatniej chwili', 'ktoś cię łapie'). "
        f"Wykonaj akcję zgodnie z intencją gracza i wywołaj modify_stats z obrażeniami prowadzącymi do HP=0. "
        f"Śmierć jest ważnym elementem RPG — pozwól jej zaistnieć, gdy gracz świadomie ją wybiera.\n"
        f"4.6. Przy śmierci z PRZYPADKU (walka, pułapka, zasadzka): wcześniej sygnalizuj rosnące niebezpieczeństwo "
        f"przez kilka tur, żeby gracz miał szansę zareagować.\n"
        f"4.7. Gdy akcja logicznie leczy (odpoczynek, medykament, naprawa) – przywróć HP przez `modify_stats` z DODATNIM hp_change.\n\n"
        ...
        f"6.2 Gdy HP bohatera po wywołaniu modify_stats osiągnie 0 lub mniej zakończ opowieść, opisz śmierć postaci.\n"
        f"Gdy bohater wciąż żyje (HP > 0), opowieść TRWA — po narracji ZAWSZE podawaj opcje A/B/C.\n\n"
        ...
        f"9.1. ⚠️ STAN GRY JEST AKTUALNY — AKTUALNY STAN BOHATERA powyżej odzwierciedla wszystkie zmiany "
        ...
        f"9.2. Masz pełną władzę nad statystykami i przedmiotami gracza przy użyciu dostarczonych narzędzi.\n"
        f"9.3. Wywołaj `modify_stats` gdy: gracz traci/zyskuje HP, traci/zarabia {character.get('currency_name', 'kredytów')}, "
        ...
        f"9.4. ⚠️ ZNALAZŁ PRZEDMIOT = add_inventory_item (ZAWSZE). Jeśli gracz w BIEŻĄCEJ turze znajduje, kupuje lub otrzymuje "
        ...
        f"9.5. Jeśli gracz próbuje użyć przedmiotu, którego nie ma w Ekwipunku, opisz niepowodzenie z powodu braku zasobów.\n\n"
